...
void sendAckFrame(uint8_t header, const uint8_t *payload, size_t n);
...
// ---- Helper Functions for u16, Decode to sign and copy values in u16
// format----
...
// ----- Helper function to send u16 frame for POS,SPD,VEL,CURR and sendACK
// Packet ----
...
void sendAllState(void *)
{
    while (1) {
        ...
        u.pack.header[2] = 0xA7;
        u.pack.header[3] = 0xA8;
        xSemaphoreTake(gMetricsMux, portMAX_DELAY);
        copy7_u16(u.pack.payload, gMetrics.pos);
        copy7_u16(u.pack.payload + 7, gMetrics.vel);
...
static bool handleHostFrame(RxSerialPack pack)
{
    ...
    case CTRL_POS: {
        ...
        hlscl.SyncWritePosEx((uint8_t *)SERVO_IDS, 7, pos, g_speed, g_accel,
                             torque_eff);
        if (gBusMux)
            xSemaphoreGive(gBusMux);
        return true;
...
void HandSetUp()
{
    ...
    ESP_LOGI("HAND", "[BOOT] Hand Type: LEFT_HAND");
#elif defined(RIGHT_HAND)
    ESP_LOGI("HAND", "[BOOT] Hand Type: RIGHT_HAND");
#else
